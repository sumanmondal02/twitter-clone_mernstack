...
function FollowModal({
  ...
  type,
  closeModal,
  isOwnProfile, // NEW PROP — pass this from Profile.jsx
}) {
  ...
  const handleAction = async (e, user) => {
    ...
    if (type === "followers" && isOwnProfile) {
      // Own followers list → Remove button
      await removeFollowerInModal(user._id);
    } else if (type === "following" && isOwnProfile) {
      // Own following list → Unfollow
      await unfollowUserInModal(user._id);
    } else {
      // Viewing someone else's list → Follow/Unfollow toggle
      if (user.isFollowing) {
        await unfollowUserInModal(user._id);
      } else {
  ...
  return (
    <div
      onClick={closeModal}
      className="fixed inset-0 z-[9999] bg-black/50 backdrop-blur-sm flex justify-center text-white pt-[7vh] items-start overflow-y-auto overflow-x-hidden sm:px-4"
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="relative w-full sm:max-w-[600px] h-screen sm:h-auto sm:max-h-[80vh] sm:min-h-[200px] bg-[#101010cf] border mb-3 border-[#2f3336] rounded-2xl overflow-hidden flex flex-col"
      >
        {/* HEADER */}
        ...
        <div className="p-4 border-b border-[#2f3336]">
          <input
            ...
            className="w-full h-[44px] rounded-full bg-[#16181c] border border-[#2f3336] px-4 text-white caret-white focus:border-[#1d9bf0] outline-none text-[15px] placeholder:text-[#71767b]"
          />
        </div>
        ...
        <div className="overflow-y-auto flex-1 text-white">
          {filteredUsers.length === 0 && (
            <div className="text-center text-[#71767b] py-16 text-[15px]">
              No users found
            </div>
          )}
